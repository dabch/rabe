...
pub struct MemorySpace {
    x: [u8; core::mem::size_of::<Yct14AbePublicKey>()],
    y: [u8; core::mem::size_of::<[Yct14Attribute;3]>()]
}
...
pub extern "C" fn rabe_yct14_init<'attname, 'attlist>(place_atts_here: *mut [Yct14Attribute<'attname>; 3], place_pubkey_here: *mut Yct14AbePublicKey<'attname, 'attlist>) -> i32 {
    ...
    let g = serde_cbor::de::from_slice_with_scratch(G_SER, &mut scratch).unwrap();
    let a: Yct14Attribute<'_> = serde_cbor::de::from_slice_with_scratch(A_SER, &mut scratch).unwrap();
    let b: Yct14Attribute<'_> = serde_cbor::de::from_slice_with_scratch(B_SER, &mut scratch).unwrap();
    let c: Yct14Attribute<'_> = serde_cbor::de::from_slice_with_scratch(C_SER, &mut scratch).unwrap();

    unsafe { core::ptr::write(place_atts_here, [a, b, c]) };
