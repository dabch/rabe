...
pub extern "C" fn rabe_yct14_partial_encrypt_init<'a, 'b>(
    ...
    atts_buf: *mut u8, // watch out, rust chars are 4 byte!! Separated by spaces.
    atts_len: usize,
    put_key_encapsulation_here: *mut Yct14AbeReusableKeyEncapsulation<'a>
) -> i32 {
    let p = match hal::pac::Peripherals::take() {
        Some(p) => p,
        None => return 0,
...
pub extern "C" fn rabe_yct14_partial_encrypt_payload_and_serialize<'a>(
    encapsulated_aes_key: *const Yct14AbeReusableKeyEncapsulation<'a>,
    plain_buf: *mut u8,
    plain_len: usize,
    ct_buf: *mut u8,
    ct_len: usize,
...
/// Creates a symmetric key and encrypts it under ABE.
/// 
/// Takes a list of attributes and encrypts a random symmetric key under yct14 KP-ABE. Then CBOR-serializes
/// the ABE-encapsulation information so that it can be sent by network. Returns a 32-byte key to be used
/// for encrypting the payload with AES-CCM.
/// 
/// @param params Yct14AbePublicKey obtained by `rabe_yct14_init()`
/// @param aes_key pointer to a buffer for the 32-byte AES key (use this to encrypt the payload; do not share!). The data in this buffer is modified.
/// @param cbor_{buf, len} memory location and length where the serialized key encapsulation shall be written. The data in this buffer is modified. Space required: About 550 bytes per attribute.
/// @param atts_{buf, len} attribute set for encryption as with `rabe_yct14_encrypt_and_serialize()`
/// @return 0 if there was an error. Otherwise returns the number of bytes written into cbor_buf.
#[no_mangle]
pub extern "C" fn rabe_yct14_key_encapsulation_and_serialize<'a, 'b>(
    params: *const Yct14AbePublicKey<'a, 'b>,
    aes_key: *mut [u8; 32],
    cbor_buf: *mut u8,
    cbor_len: usize,
    ...
    let atts: &mut str = unsafe { core::str::from_utf8_unchecked_mut(atts_u8) };
    let atts_array: Vec<&str, S> = atts.split(' ').collect();
    let cbor_buf = unsafe { slice::from_raw_parts_mut(cbor_buf, cbor_len) };


    let (encapsulation, key) = match create_and_extract_symmetric_key(&params, &atts_array, &mut rng) {
        Ok((e, k)) => (e, k),
        Err(_) => return 0,
    };
    ...
    let writer = SliceWrite::new(cbor_buf);
    let mut serializer = Serializer::new(writer);

    match encapsulation.serialize(&mut serializer) {
        Ok(_) => (),
        Err(_) => return 0,
    };
...
mod test {
    ...
    #[test]
    fn print_sizeof() {
        println!("sizeof(Yct14AbeKeyEncapsulation) = {}", core::mem::size_of::<Yct14AbeReusableKeyEncapsulation>());
