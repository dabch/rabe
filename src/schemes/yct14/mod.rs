...
/// A state containing an ABE-encrypted curve point (used as AES key) and the key encapsulation (allows the secret to be reconstructed under ABE)
#[derive(Debug, PartialEq)]
pub struct Yct14AbeReusableKeyEncapsulation<'name> {
    attributes: Vec<Yct14Attribute<'name>, S>,
    secret: Gt,
}

/// The same as Yct14AbeReusableKeyEncapsulation, but without the secret (this will be sent over the network)
#[derive(Debug, PartialEq, Serialize)]
pub struct Yct14AbeKeyEncapsulation<'name>(Vec<Yct14Attribute<'name>, S>);
...
pub fn encrypt<'attname, 'attlist, 'data>(
    ...
    if _attributes.is_empty() {
        ...
        let Yct14AbeReusableKeyEncapsulation { attributes, secret } = create_symmetric_key(_pk, _attributes, _rng)?;
...
pub fn encrypt_using_symmetric_key<'attname, 'data>(
    _key: &Yct14AbeReusableKeyEncapsulation<'attname>,
    _plaintext: &'data mut [u8],
    _rng: &mut dyn RngCore,
) -> Result<Yct14AbeCiphertext<'attname, 'data>, RabeError<'static>> {
    let Yct14AbeReusableKeyEncapsulation { attributes, secret } = _key;
...
pub fn create_symmetric_key<'attname, 'attlist>(
    ...
    _attributes: &'attlist [&'attname str],
    _rng: &mut dyn RngCore,
) -> Result<Yct14AbeReusableKeyEncapsulation<'attname>, RabeError<'static>> {
    if _attributes.is_empty() {
        return Err(RabeError::new("attributes empty"));
    }
    // attribute vector
    ...
    // Encrypt plaintext using aes secret
    Ok(Yct14AbeReusableKeyEncapsulation{ attributes, secret: _cs})
}

pub fn create_and_extract_symmetric_key<'attname, 'attlist>(
    _pk: &Yct14AbePublicKey,
    _attributes: &'attlist [&'attname str],
    _rng: &mut dyn RngCore,
) -> Result<(Yct14AbeKeyEncapsulation<'attname>, [u8; 32]), RabeError<'static>> {
    let with_secret = create_symmetric_key(_pk, _attributes, _rng)?;
    let hashed_key = kdf(&with_secret.secret);
    Ok((Yct14AbeKeyEncapsulation(with_secret.attributes), hashed_key.into()))
}
/// # Arguments
///
